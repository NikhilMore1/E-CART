...
function Beauty() {
    ...
    const fetchData = async () => {
        try {
            const resp = await axios.get("http://localhost:5000/api/Beauty");
    ...
    <div className="row mt-3" style={{ margin: '0 auto', padding: '40px', width: '100%',backgroundColor:'rgb(231, 238, 208)' }}>
    {
        watch.map(product=>(
            ...
            <div className="col-md-3 mt-3">
               <div className="card">
<Link to={`/BeautyDetails/${product._id}`}><img src={product.image} className="card-img-top" alt={product.Heading} height="250"/></Link>
